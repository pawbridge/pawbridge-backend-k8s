...
`application-postgresql.yml`은 `PAYMENT_POSTGRESQL_JDBC_URL`,
`PAYMENT_POSTGRESQL_USERNAME`, `PAYMENT_POSTGRESQL_PASSWORD`를 명시한 경우에만 사용한다.
기존 기본 MySQL 프로필은 유지한다. Hikari는 서비스별 풀(기본 최대 5, 최소 0),
스키마 `pawbridge_payment`, Hibernate `validate`, OSIV 비활성화를 사용한다.
...
`PAYMENT_PG_MIGRATION_JDBC_URL`, `PAYMENT_PG_MIGRATION_USERNAME`,
`PAYMENT_PG_MIGRATION_PASSWORD`를 사용한다.
URL은 `jdbc:postgresql://127.0.0.1:<port>/pawbridge` 형식의 루프백 대상만 허용한다.
DB 관리자가 빈 `pawbridge_payment` 스키마를 먼저 준비해야 하며,
`schemaPostgresqlMigrate`에는 URL과 정확히 같은 `PAYMENT_PG_MIGRATION_CONFIRM_TARGET`이 필요하다.
`schemaPostgresqlInfo`, `schemaPostgresqlValidate`도 제공한다.
데이터 적재·운영 계정·CDC·운영 접속 URL 확장은 별도 전환 작업이다.

실제 DB 검증은 `PAYMENT_PG_MIGRATION_TEST_PORT`와 전용 DB가 필요하다.
