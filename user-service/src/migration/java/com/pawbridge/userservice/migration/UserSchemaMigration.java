...
public final class UserSchemaMigration {
    ...
    static final class Settings {
        ...
        static Settings from(Map<String, String> env) {
            String url = required(env, "USER_MIGRATION_JDBC_URL");
            // No userinfo, query credentials, multi-host URLs or cross-service database names.
            if (!url.matches("jdbc:mysql://[a-zA-Z0-9.-]+:[0-9]{1,5}/pawbridge_user")) {
                throw new IllegalArgumentException("Expected one explicit MySQL host, port and user schema");
            }
            return new Settings(url, required(env, "USER_MIGRATION_USERNAME"),
                    required(env, "USER_MIGRATION_PASSWORD"),
                    url.equals(env.get("USER_MIGRATION_CONFIRM_TARGET")));
        }
