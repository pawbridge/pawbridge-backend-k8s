...
public final class CommunitySchemaMigration {
    static final String SCHEMA = "pawbridge_community";
    private static final Set<String> COMMANDS = Set.of("inspect", "info", "validate", "migrate");

    private CommunitySchemaMigration() { }
    ...
    static final class Settings {
        ...
        static Settings from(Map<String, String> env) {
            String url = required(env, "COMMUNITY_MIGRATION_JDBC_URL");
            // No userinfo, query credentials, multi-host URLs or cross-service database names.
            if (!url.matches("jdbc:mysql://[a-zA-Z0-9.-]+:[0-9]{1,5}/pawbridge_community")) {
                throw new IllegalArgumentException("Expected one explicit MySQL host, port and community schema");
            }
            return new Settings(url, required(env, "COMMUNITY_MIGRATION_USERNAME"),
                    required(env, "COMMUNITY_MIGRATION_PASSWORD"),
                    url.equals(env.get("COMMUNITY_MIGRATION_CONFIRM_TARGET")));
        }
