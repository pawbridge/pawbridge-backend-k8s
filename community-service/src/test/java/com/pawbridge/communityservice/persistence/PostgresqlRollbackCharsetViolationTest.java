package com.pawbridge.communityservice.persistence;
...
class PostgresqlRollbackCharsetViolationTest {
    ...
    void charset_constraint_returns_400_without_exposing_sql_or_row_data() throws Exception {
        PSQLException failure = error("23514", "ck_rollback_charset_example", "pawbridge_community");
    ...
    void other_constraint_state_schema_and_message_text_do_not_become_user_errors() throws Exception {
        assertThat(PostgresqlRollbackCharsetViolation.matches(error("23505", "ck_rollback_charset_example", "pawbridge_community"))).isFalse();
        assertThat(PostgresqlRollbackCharsetViolation.matches(error("23514", "business_constraint", "pawbridge_community"))).isFalse();
        ...
        mvc(new DataIntegrityViolationException("fixture", error("23514", "business_constraint", "pawbridge_community")))
    ...
    void batched_sql_exception_retains_the_structured_constraint_identity() {
        ...
        batch.setNextException(error("23514", "ck_rollback_charset_example", "pawbridge_community"));
