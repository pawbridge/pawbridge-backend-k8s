...
public final class AnimalSchemaMigration {
    static final String SCHEMA = "pawbridge_animal";
    private static final Set<String> COMMANDS = Set.of("inspect", "info", "validate", "migrate");

    private AnimalSchemaMigration() { }
    ...
    static final class Settings {
        ...
        static Settings from(Map<String, String> env) {
            String url = required(env, "ANIMAL_MIGRATION_JDBC_URL");
            // No userinfo, query credentials, multi-host URLs or cross-service database names.
            if (!url.matches("jdbc:mysql://[a-zA-Z0-9.-]+:[0-9]{1,5}/pawbridge_animal")) {
                throw new IllegalArgumentException("Expected one explicit MySQL host, port and animal schema");
            }
            return new Settings(url, required(env, "ANIMAL_MIGRATION_USERNAME"),
                    required(env, "ANIMAL_MIGRATION_PASSWORD"),
                    url.equals(env.get("ANIMAL_MIGRATION_CONFIRM_TARGET")));
        }
