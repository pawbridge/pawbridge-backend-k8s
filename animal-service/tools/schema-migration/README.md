...
3. 국문 관광정보 서비스 키를 `TOURAPI_BULKSERVICEKEY`로 주입한다. 기존 `TOURAPI_SERVICEKEY`는 반려동물 동반여행 서비스용으로 유지한다.
...
일괄 수집은 국문 `KorService2/detailPetTour2`를 ID 없이 페이지당 100건씩 요청한다.
`TOURAPI_MAXBULKPAGESPERRUN`의 기본값은 10이다. 저장한 페이지 다음부터 재개하며, 한 순회를 마치면 24시간 동안 새 순회를 시작하지 않는다.
목록·표출 여부와 공통 상세는 기존 `KorPetTourService2`에서 계속 수집한다. 공통 상세는 여전히 장소별 요청이다.
...
- 일괄 방식만 중단: 수집을 멈춘 뒤 `TOURAPI_BULKPETENABLED=false`로 전환한다. 새 코드에서는 기존 독립 상세를 계속 조회하며, 이후 공통 상세 작업부터 개별 동반 요청을 다시 사용한다. 이미 처리된 모든 장소가 즉시 재수집되는 것은 아니다.
