...
명령은 Java 17에서 `store-service` 디렉터리를 기준으로 실행한다.

## 현재 범위

V1은 2026-09-11 운영 MySQL 8.4.12의 `pawbridge_store` 테이블 12개를 읽기 전용으로 확인해 작성했다. 뷰·트리거·루틴·이벤트는 없었다. 운영 행과 자동증가 카운터는 포함하지 않는다. 기존 문자셋과 제약조건을 보존한다.
...
java -cp 'build/migration/lib/*' com.pawbridge.storeservice.migration.StoreSchemaMigration info
```

다음 환경변수를 승인된 환경에서 주입한다. 운영 비밀번호를 명령 인자나 파일에 적지 않는다.

- `STORE_MIGRATION_JDBC_URL`: `jdbc:mysql://<host>:<port>/pawbridge_store`
- `STORE_MIGRATION_USERNAME`: 이 스키마 전용 계정
- `STORE_MIGRATION_PASSWORD`: 계정 비밀번호
- `STORE_MIGRATION_CONFIRM_TARGET`: 변경 승인 후에만 JDBC URL과 같은 값
...
운영 DB나 운영 포트포워딩을 연결하지 않는다. 전용 컨테이너에 빈 `pawbridge_store`를 만든다. 테스트 root 비밀번호는 `local_flyway_test_only`다. 실제 계정에 사용하지 않는다.

```sql
CREATE DATABASE IF NOT EXISTS flyway_test_guard;
CREATE TABLE flyway_test_guard.store_guard (marker VARCHAR(64) NOT NULL);
INSERT INTO flyway_test_guard.store_guard VALUES ('store-flyway-disposable');
```

`STORE_MIGRATION_TEST_PORT`에 전용 로컬 포트를 지정한다. Java 컨테이너가 전용 MySQL 컨테이너의 네트워크를 공유하면 3306이다.

```bash
bash ./gradlew --no-daemon migrationMysqlTest
```

보호 마커 확인 후 이 서비스의 고정 테이블 목록과 테스트 테이블·이력만 초기화한다. 다른 MS 테이블은 초기화하지 않는다. V1 생성, JPA 엔티티 12개 호환성, SQL 누락과 기존 스키마 거절을 검증한다. 기존 행 보존 baseline 시험은 `migrationRehearsal`로 따로 실행한다.
...
`application-postgresql.yml`은 `STORE_POSTGRESQL_JDBC_URL`,
`STORE_POSTGRESQL_USERNAME`, `STORE_POSTGRESQL_PASSWORD`를 명시한 경우에만 사용한다.
기존 기본 MySQL 프로필은 유지한다. Hikari는 서비스별 풀(기본 최대 5, 최소 0),
스키마 `pawbridge_store`, Hibernate `validate`, OSIV 비활성화를 사용한다.
...
`STORE_PG_MIGRATION_JDBC_URL`, `STORE_PG_MIGRATION_USERNAME`,
`STORE_PG_MIGRATION_PASSWORD`를 사용한다.
URL은 `jdbc:postgresql://127.0.0.1:<port>/pawbridge` 형식의 루프백 대상만 허용한다.
DB 관리자가 빈 `pawbridge_store` 스키마를 먼저 준비해야 하며,
`schemaPostgresqlMigrate`에는 URL과 정확히 같은 `STORE_PG_MIGRATION_CONFIRM_TARGET`이 필요하다.
`schemaPostgresqlInfo`, `schemaPostgresqlValidate`도 제공한다.
데이터 적재·운영 계정·CDC·운영 접속 URL 확장은 별도 전환 작업이다.

실제 DB 검증은 `STORE_PG_MIGRATION_TEST_PORT`와 전용 DB가 필요하다.
