...
public final class KoreanSearchTerms {
    ...
    public String query(String text) {
        ...
        if (text.length()>200 || text.indexOf(0)>=0) throw new IllegalArgumentException("검색어는 200자 이내여야 합니다.");
        List<String> terms = new ArrayList<>(new LinkedHashSet<>(tokens(text)));
        if (terms.size()>32) throw new IllegalArgumentException("검색 단어가 너무 많습니다.");
