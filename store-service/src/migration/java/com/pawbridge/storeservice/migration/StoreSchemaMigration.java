...
public final class StoreSchemaMigration {
    static final String SCHEMA = "pawbridge_store";
    private static final Set<String> COMMANDS = Set.of("inspect", "info", "validate", "migrate");

    private StoreSchemaMigration() { }
    ...
    static final class Settings {
        ...
        static Settings from(Map<String, String> env) {
            String url = required(env, "STORE_MIGRATION_JDBC_URL");
            // No userinfo, query credentials, multi-host URLs or cross-service database names.
            if (!url.matches("jdbc:mysql://[a-zA-Z0-9.-]+:[0-9]{1,5}/pawbridge_store")) {
                throw new IllegalArgumentException("Expected one explicit MySQL host, port and store schema");
            }
            return new Settings(url, required(env, "STORE_MIGRATION_USERNAME"),
                    required(env, "STORE_MIGRATION_PASSWORD"),
                    url.equals(env.get("STORE_MIGRATION_CONFIRM_TARGET")));
        }
